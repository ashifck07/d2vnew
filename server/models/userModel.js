...
//     console.log('Expires At:', userDoc.expiresAt);
//     const expiresAt = new Date(userDoc.expiresAt).getTime();

//     if (userDoc.otp !== otp || currentTime > expiresAt) {
//       throw new Error('Invalid or expired OTP.');
//     }

//     console.log('OTP Expiration Check:', new Date(userDoc.expiresAt) < new Date());

//     // Reset OTP to null after successful validation
//     await db.collection('users').doc(userDoc.uid).update({ otp: null });
//     return userDoc; // Return user data if OTP validation is successful
//   } catch (error) {
...
async function loginUser(phoneNumber, otp) {
  try {
    ...
    console.log("Stored OTP:", userData.otp); // Log stored OTP
